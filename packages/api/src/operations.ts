...
import { listTables } from "./reservations/logic.js";
...
export async function getFloorState(args: {
  ...
  return tables.map((t) => {
    ...
    const reserved = Boolean(r) && !occupied;
...
export async function updateReservation(args: {
  ...
  if (input.partySize != null) {
    ...
    if (!table || table.capacity < nextParty) return { ok: false, reason: "CONFLICT" };
  }
...
export async function runReminderSweep(args: {
  ...
  const target = addHours(now, REMINDER_LEAD.hours);
